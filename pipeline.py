...
def inference(meta, args):
    ...
    sdxl_base = DiffusionPipeline.from_pretrained(
        "/home/minkai/video_generation/models/stable-diffusion-xl-base-1.0/", torch_dtype=torch.float16, variant="fp16", use_safetensors=True
    ).to("cuda")
    sdxl_refiner = DiffusionPipeline.from_pretrained(
        "/home/minkai/video_generation/models/stable-diffusion-xl-refiner-1.0/", text_encoder_2=sdxl_base.text_encoder_2, vae=sdxl_base.vae, torch_dtype=torch.float16, use_safetensors=True, variant="fp16",
    ).to("cuda")
    pipeline = I2VGenXLPipeline.from_pretrained("/home/minkai/video_generation/models/i2vgen-xl/", torch_dtype=torch.float16, variant="fp16").to("cuda")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fps", type=int, default=4, help="fps of the generated video")
    args = parser.parse_args()

    meta = dict(
                prompt = ["A hot air balloon flying from left to right in a blue sky"],
                llm_prompt = ["A large hot air balloon flying from left to right in a blue sky",
                              "A large hot air balloon flying from left to right in a blue sky",
                              "A large hot air balloon flying from left to right in a blue sky"],
                negative_prompt = "Distorted, discontinuous, Ugly, blurry, low resolution, motionless, static, disfigured, disconnected limbs, Ugly faces, incomplete arms",
                foldername = 'generation/balloon/'
            )   
